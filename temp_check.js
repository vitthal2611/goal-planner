    ...
    function updateBalanceSummary() {
      ...
      totalIncomeEl.textContent = `â‚¹${income.toLocaleString('en-IN')}`;
      totalExpenseEl.textContent = `â‚¹${expense.toLocaleString('en-IN')}`;
    }
      ...
      container.innerHTML = paymentMethods.map((method, index) => {
        ...
        return `
          <div class="payment-item ${colorClass}">
            ...
            <div class="payment-value">â‚¹${balance.toLocaleString('en-IN')}</div>
          </div>
        `;
      }).join('');
    }
    ...
    function updateRecentTransactions() {
      ...
      function renderTxItem(t) {
        ...
        const amount = t.type === 'transfer'
          ? `${t.from}â†’${t.to} â‚¹${parseFloat(t.amount).toLocaleString('en-IN')}`
          : `${sign}â‚¹${parseFloat(t.amount).toLocaleString('en-IN')}`;
    ...
    function updateEnvelopeBudget() {
      ...
      if (needTotalTopEl) needTotalTopEl.textContent = `â‚¹${needTotal.toLocaleString('en-IN')}`;
      if (wantTotalTopEl) wantTotalTopEl.textContent = `â‚¹${wantTotal.toLocaleString('en-IN')}`;
      if (saveTotalTopEl) saveTotalTopEl.textContent = `â‚¹${saveTotal.toLocaleString('en-IN')}`;
      ...
      container.innerHTML = envelopes.map(envelope => {
        ...
        let breakdownParts = [];
        if (needSpent > 0) breakdownParts.push(`ðŸŽ¯ â‚¹${needSpent.toLocaleString('en-IN')}`);
        if (wantSpent > 0) breakdownParts.push(`ðŸŽ‰ â‚¹${wantSpent.toLocaleString('en-IN')}`);
        if (saveSpent > 0) breakdownParts.push(`ðŸ’° â‚¹${saveSpent.toLocaleString('en-IN')}`);
        ...
        const remainingText = remaining >= 0
          ...
          : `â‚¹${Math.abs(remaining).toLocaleString('en-IN')} over`;

        const chips = [
          needSpent  > 0 ? `<span class="nws-chip need">ðŸŽ¯ â‚¹${needSpent.toLocaleString('en-IN')}</span>`  : '',
          wantSpent  > 0 ? `<span class="nws-chip want">ðŸŽ‰ â‚¹${wantSpent.toLocaleString('en-IN')}</span>`  : '',
          saveSpent  > 0 ? `<span class="nws-chip save">ðŸ’° â‚¹${saveSpent.toLocaleString('en-IN')}</span>`  : '',
        ].filter(Boolean).join('');
        ...
        return `
          <div class="envelope-item">
            ...
            <div class="envelope-amounts">â‚¹${actualSpent.toLocaleString('en-IN')} / â‚¹${budgetAmount.toLocaleString('en-IN')} &nbsp;Â·&nbsp; ${remainingText}</div>
      ...
      if (methodName) {
        if (!paymentMethods.includes(methodName)) {
          try {
            ...
            updatePaymentBalances();
            syncToCloud();
            const balanceMsg = balance ? ` with initial balance â‚¹${balance}` : '';
      ...
      if (envelopeName) {
        if (!envelopes.includes(envelopeName)) {
          try {
            ...
            updateEnvelopeDropdowns();
            syncToCloud();
            const budgetMsg = budget ? ` with default budget â‚¹${budget}` : '';
      ...
      document.getElementById('csvPreviewCards').innerHTML = preview.map(r => {
        ...
        return `<div style="background:${bg}; border-radius:12px; padding:12px 14px; display:flex; justify-content:space-between; align-items:center; gap:8px;">
          ...
          <div style="text-align:right; flex-shrink:0;">
            <div style="font-size:15px; font-weight:800; color:${tc};">â‚¹${parseFloat(r.amount).toLocaleString('en-IN')}</div>
      ...
      if (activeType === 'expense') {
        ...
        if (successCount > 0) {
          ...
          const message = successCount > 1 
            ? `${successCount} expenses added: â‚¹${totalAmount.toLocaleString('en-IN')}`
            : `Expense: â‚¹${totalAmount.toLocaleString('en-IN')}`;
        ...
        if (amount && fromPayment && toPayment) {
          ...
          try {
            ...
            updatePaymentBalances();
            updateEnvelopeBudget();
            const message = `Transfer: â‚¹${amount} from ${fromPayment} to ${toPayment}`;
            showToast(message, 'success');
            transactionModal.classList.remove('show');
            expenseEntriesContainer.innerHTML = '';
            expenseEntryCount = 0;
        ...
        if (amount && description && payment) {
          ...
          try {
            ...
            updatePaymentBalances();
            updateEnvelopeBudget();
            const message = `Income: â‚¹${amount} - ${description}`;
            showToast(message, 'success');
            transactionModal.classList.remove('show');
            expenseEntriesContainer.innerHTML = '';
            expenseEntryCount = 0;
